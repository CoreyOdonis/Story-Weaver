...
export default function Home() {
  ...
  const [generatedStory, setGeneratedStory] = useState<{ title: string; story: string; emoji: string; childName: string } | null>(null);
  ...
  const form = useForm<z.infer<typeof formSchema>>({ resolver: zodResolver(formSchema), defaultValues: { childName: "", age: 5, interests: [], length: "5min" } });
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    generateStoryMutation.mutate({ data: { childName: selectedChild.name, age: selectedChild.age ?? values.age, interests: selectedChild.interests as GenerateStoryRequestInterestsItem[], storyLength: values.length, ...(selectedTone ? { tone: selectedTone } : {}) } }, { onSuccess: (result) => { setGeneratedStory({ title: result.title, story: result.story, emoji: result.emoji, childName: selectedChild.name }); recordActivity(); if (firebaseUser) { void savePreferences({ childName: selectedChild.name, age: selectedChild.age ?? values.age, interests: selectedChild.interests, storyLength: values.length }); } } });
  };

  const handleContinueStory = async () => {
    if (!selectedChild || !selectedSeries) return;
    const lastStory = savedStories?.filter((story) => story.seriesId === selectedSeries.id).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
    if (!lastStory) return;
    ...
    try {
      ...
      const result = (await response.json()) as { title: string; story: string; emoji: string };
  ...
  const handleReset = () => { setGeneratedStory(null); setSavedThisSession(false); setPendingInterests(""); resetPdf(); form.reset({ childName: "", age: 5, interests: [], length: "5min" }); setSelectedChildId(null); setSelectedChild(null); setSelectedTone(null); setSelectedSeriesId(null); setSelectedSeries(null); };

  const paragraphs = generatedStory?.story.split(/\n\n+/).map((p) => p.trim()).filter(Boolean) ?? [];
  const estimatedReadMinutes = useMemo(() => { if (!generatedStory?.story) return 5; const words = generatedStory.story.trim().split(/\s+/).filter(Boolean).length; return Math.max(1, Math.round(words / WORDS_PER_MINUTE)); }, [generatedStory?.story]);

  useEffect(() => {
    if (!generatedStory) { setStoryProgress(0); return; }
    const updateProgress = () => { const storyArea = document.getElementById("story-reading-area"); if (!storyArea) return; const rect = storyArea.getBoundingClientRect(); const total = rect.height + window.innerHeight; const scrolled = Math.min(total, Math.max(0, window.innerHeight - rect.top)); setStoryProgress(Math.max(0, Math.min(100, (scrolled / total) * 100))); };
    updateProgress(); window.addEventListener("scroll", updateProgress, { passive: true }); window.addEventListener("resize", updateProgress); return () => { window.removeEventListener("scroll", updateProgress); window.removeEventListener("resize", updateProgress); };
  }, [generatedStory]);
  ...
  return (
    <div className="min-h-[100dvh] w-full relative overflow-hidden flex flex-col items-center py-10 px-4 sm:px-8">
      <div className="relative z-10 w-full max-w-4xl mx-auto mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2"><span className="text-2xl">🌙</span><span className="font-serif font-bold text-lg text-foreground/90 hidden sm:block">Dreamtime Stories</span></div>
        {authLoading ? null : firebaseUser ? (<div className="flex items-center gap-3"><span className="text-sm text-foreground/80 hidden sm:block">{profile?.name ?? firebaseUser.displayName}</span><Button variant="outline" size="sm" className="rounded-full border-white/10 text-xs h-8 px-3" onClick={signOut}>Sign out</Button></div>) : (<Button variant="outline" size="sm" className="rounded-full border-white/20 bg-white/5 hover:bg-white/10 text-sm h-9 px-4 gap-2" onClick={signInWithGoogle}>Continue with Google</Button>)}
      </div>

      {firebaseUser && (<ChildProfileBar children={children} selectedChildId={selectedChildId} onSelect={handleSelectChild} onCreate={async (data) => { await createChild(data); }} onUpdate={async (id, data) => { await updateChild(id, data); }} onDelete={async (id) => { await deleteChild(id); }} />)}
